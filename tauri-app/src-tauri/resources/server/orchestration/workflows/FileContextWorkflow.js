...
class FileContextWorkflow {
  async run(context) {
    ...
    const MAX_BYTES = maxBytes || 65536;
    const MAX_FILE_BYTES = 51200; // 50KB per-file cap
    let totalBytes = 0;
    ...
    const allFiles = fs.readdirSync(projectDir).filter(f => {
      const full = path.join(projectDir, f);
      return fs.statSync(full).isFile() && !f.startsWith(".protoai-");
    });
